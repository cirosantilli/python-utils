...
used for stuff that refers to the items owned by the user:
    repos, etc

- item2: item that is owned by user
- item3: if item2 is a list, item3 are items in that list

exception: id2, used because id already taken
"""
...
@require_safe
def index(request, owner_username):
...
def index_all(request):
    ...
    return render(
        request,
        'user_user_groups/index_all.html',
        {
            'total_items_db': all_items_db.count(),
            'table': table,
            'table_filter': table_filter,
...
def detail(request, owner_username, id2):
    ...
    bulk_form_id = 'bulk' #<input form=> and <form id=>
...
class UserGroupForm(ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if initial_usergroup: 
            ...
            kwargs['initial'] = {}
            kwargs['initial'].update(model_to_dict(initial_usergroup,fields=['id2']))
...
def create(request, owner_username):
    ...
    if request.method == "POST":
        form = UserGroupForm(request.POST, owner=owner)
        if form.is_valid():
            ...
            return HttpResponseRedirect(reverse('user_user_groups_index',args=(owner_username,)))
    else:
        form = UserGroupForm()

    return render(
        request,
        "user_user_groups/create_item2.html",
        {
            "form": form,
            "owner": owner,
        },
...
@login_required
def update2(request, owner_username, id2):
    ...
    if request.method == "POST":
        form = UserGroupForm(request.POST, owner=owner, old_id2=item2.id2)
        if form.is_valid():
        ...
        form = UserGroupForm(initial_usergroup=item2)

    return render(
        request,
        "user_user_groups/update_item2.html",
        {
            'form': form,
            'owner': owner,
            'item2': item2,
...
def bulk_action2(request, owner_username):

    if request.POST.__contains__('remove'):
        return remove_selected2(request,owner_username,bid)
    else:
        return HttpResponseBadRequest("unknown action" % action)
